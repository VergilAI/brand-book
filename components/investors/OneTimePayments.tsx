...
export function OneTimePayments() {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      // Filter for one-time expenses only
      const oneTimeExpenses = data.filter(item => item.transaction_type === "onetime");
      setExpenses(oneTimeExpenses);
    } catch (error) {
        ...
        {loading ? (
          ...
          <div className="max-h-80 overflow-y-auto">
            <div className="p-4 space-y-2">
              {expenses.length === 0 ? (
                <p className="text-gray-500 text-center py-8">No one-time payments</p>
              ) : (
