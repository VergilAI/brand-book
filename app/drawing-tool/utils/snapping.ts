import { Point, Line, SnapResult, SnapSettings } from '../types/snapping'
import { BezierPoint, Shape } from '../types/drawing'
...
export function findNearestVertex(
  ...
  shapes.forEach(shape => {
    ...
    const vertices = parseSvgPathToPoints(shape.fillPath)
...
export function findNearestCenter(
  ...
  shapes.forEach(shape => {
    ...
    const dist = distance(point, shape.center)
    if (dist < minDist) {
      minDist = dist
      nearest = {
        point: shape.center,
...
export function getSnapCandidates(
  ...
  if (settings.vertexSnap) {
    const vertex = findNearestVertex(point, shapes, settings.snapDistance, excludeShapeId)
    if (vertex) candidates.push(vertex)
  }

  if (settings.edgeSnap) {
    const edge = findNearestEdgePoint(point, shapes, settings.snapDistance, excludeShapeId)
    if (edge) candidates.push(edge)
  }

  if (settings.centerSnap) {
    const center = findNearestCenter(point, shapes, settings.snapDistance, excludeShapeId)
...
export function getAlignmentGuides(
  ...
  shapes.forEach(shape => {
    ...
    if (Math.abs(point.y - shape.center.y) < threshold) {
      const lineKey = `h-${shape.center.y}`
      if (!processedLines.has(lineKey)) {
        processedLines.add(lineKey)
        guides.push({
          start: { x: Math.min(point.x, shape.center.x) - 100, y: shape.center.y },
          end: { x: Math.max(point.x, shape.center.x) + 100, y: shape.center.y }
        })
      }
    ...
    if (Math.abs(point.x - shape.center.x) < threshold) {
      const lineKey = `v-${shape.center.x}`
      if (!processedLines.has(lineKey)) {
        processedLines.add(lineKey)
        guides.push({
          start: { x: shape.center.x, y: Math.min(point.y, shape.center.y) - 100 },
          end: { x: shape.center.x, y: Math.max(point.y, shape.center.y) + 100 }
        })
      }
      alignedElements.push({
        type: 'center',
