...
export function ContentStep({ state, onStateChange, onNext, onBack }: ContentStepProps) {
          ...
          {modules.map(module => {
            ...
            return (
              <Card key={module.id} className="p-spacing-md">
                ...
                {moduleCompleted === moduleTasks.length && selectedGames.includes('test') && (
                  <div className="mt-spacing-md pt-spacing-md border-t border-border-subtle">
                    <Button
                      variant="ghost"
                      size="md"
                      onClick={() => router.push('/lms/tests/create?from=course-generator&step=generating')}
